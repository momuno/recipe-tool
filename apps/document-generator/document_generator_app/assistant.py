...
class DocumentAssistant:
    ...
    async def execute_command(
        self, intent: CommandIntent, app_functions: Dict[str, callable], current_state: Dict[str, Any]
    ) -> Tuple[Any, str]:
        ...
        action_mapping = {
            "create_outline": lambda: app_functions["handle_start_draft_click"](
                intent.parameters.get("topic", ""), current_state.get("resources", []), current_state.get("session_id")
            ),
            ...
            "add_text_block": lambda: app_functions["add_text_block"](
                current_state.get("blocks", []), current_state.get("focused_block_id")
            ),
            "generate_document": lambda: app_functions["handle_document_generation"](
                current_state.get("blocks", []), current_state.get("resources", []), current_state.get("session_id")
            ),
            "export_docx": lambda: app_functions["handle_download_click"](current_state.get("blocks", []), "docx"),
            "export_markdown": lambda: app_functions["handle_download_click"](current_state.get("blocks", []), "md"),
...
def handle_chat_message(
    ...
    # Execute command
    result, response = asyncio.run(assistant.execute_command(intent, app_functions, context))
