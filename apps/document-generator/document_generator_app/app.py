...
def load_example(example_id, session_id=None):
    ...
    # We need to skip import_file (at index 5) from the result
    return (
        result[0],  # title
        ...
        result[3],  # blocks
        result[4],  # json_str
        result[6],  # session_id (skip import_file at 5)
        result[7],  # generated_content_html
        result[8],  # generated_content
        result[9],  # save_doc_btn
    )
...
async def handle_start_draft_click(prompt, resources, session_id=None):
    ...
    try:
        ...
        if outline_json:
            ...
            def sections_to_blocks(sections, parent_indent=-1):
                ...
                for section in sections:
                    ...
                    if "prompt" in section:
                        ...
                        # Handle refs
                        refs = section.get("refs", [])
                        if refs and processed_resources:
                            for ref in refs:
            ...
            # Generate the JSON for the outline using current state (includes proper refs)
            json_str = generate_outline_json_from_state(title, description, processed_resources, blocks)
...
def handle_file_upload(files, current_resources, title, description, blocks, session_id=None):
    ...
    # print(f"DEBUGhandle_file_upload - title: {title}, description: {description}, blocks: {blocks}")
    ...
    # print(f"DEBUGhandle_file_upload - returning json_str: {json_str[:100]}...")
    ...
    # print(f"  - json_str type: {type(json_str)}, length: {len(json_str)}")fv
...
def render_start_resources(resources):
    # print(f"DEBUGrender_start_resources called with {len(resources) if resources else 0} resources")
...
def delete_gradio_and_render(resources, path, title, desc, blocks, focused):
    ...
    new_res, new_blocks, json_str = delete_resource_gradio(resources, path, title, desc, blocks)
    blocks_html = render_blocks(new_blocks, focused)
    print("=== delete_gradio_and_render complete ===\n")
    return new_res, new_blocks, json_str, blocks_html
...
def extract_resources_from_docpack(docpack_path, session_id=None):
    ...
    # print(f"DEBUGextract_resources_from_docpack: Returning {len(resources)} resources")
    for r in resources:
        print(f"  Resource: {r}")
    return resources
...
def create_draft_document(prompt: str, resources: list = None, session_id: str = None) -> dict:
    ...
    # Check if there was an error (error message would be visible)
    if error_msg_val and hasattr(error_msg_val, "visible") and error_msg_val.visible:
        return {
            "status": "error",
            "message": "Failed to create document outline",
        }
...
def create_app():
    ...
    with gr.Blocks(title="Document Generator", css=custom_css, head=custom_js) as app:
        ...
        update_trigger.click(
            ...
            outputs=[gr_blocks_state, json_output],
            show_api=False,
        ).then(fn=set_focused_block, inputs=update_block_id, outputs=gr_focused_block_state, show_api=False)

        # Toggle collapse handler - also update the collapse/expand all button
        toggle_trigger.click(
            fn=toggle_block_collapse, inputs=[gr_blocks_state, toggle_block_id], outputs=gr_blocks_state, show_api=False
        ).then(fn=set_focused_block, inputs=toggle_block_id, outputs=gr_focused_block_state, show_api=False).then(
            fn=render_blocks, inputs=[gr_blocks_state, toggle_block_id], outputs=blocks_display, show_api=False
        ).then(
            fn=update_collapse_button_state, inputs=[gr_blocks_state], outputs=workspace_collapse_btn, show_api=False
        )
        ...
        update_heading_trigger.click(
            ...
            outputs=[gr_blocks_state, json_output],
            show_api=False,
        ).then(fn=set_focused_block, inputs=update_heading_block_id, outputs=gr_focused_block_state, show_api=False)
        ...
        indent_trigger.click(
            ...
            fn=render_blocks, inputs=[gr_blocks_state, gr_focused_block_state], outputs=blocks_display, show_api=False
        ).then(fn=set_focused_block, inputs=indent_block_id, outputs=gr_focused_block_state, show_api=False)

        # Focus handler
        focus_trigger.click(
            fn=set_focused_block, inputs=focus_block_id, outputs=gr_focused_block_state, show_api=False
        ).then(fn=render_blocks, inputs=[gr_blocks_state, focus_block_id], outputs=blocks_display, show_api=False)
        ...
        ui_start_tab_prompt_input.input(
            fn=lambda: gr.update(visible=False), outputs=[ui_start_tab_error_message], queue=False, show_api=False
        )
